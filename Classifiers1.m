...
Varia = var(Data); %variance of each feature
VarMax = sort(Varia,'descend'); VarMax=VarMax(1:2); %Taking the two highest variances
idx1 = find(Varia == VarMax(1)); D1 = Data(:,idx1); %First dimension
idx2 = find(Varia == VarMax(2)); D2 = Data(:,idx2); %Second dimension
Healthy = (Out==0); %Positions of healthy class
figure('Position',[744 108 560 420])
plot(D1(Healthy),D2(Healthy),'*b') %Plotting Data points
hold on,plot(D1(~Healthy),D2(~Healthy),'*r'),hold off
xlabel('First dimension'),ylabel('Second dimension')
title('Data points'),legend('Healthy','Parkisons')
